...
class AdminService {
  ...
  Future<int> getTotalScans() async {
    ...
    await scansRef.get().then((snapshot) {
      for (DocumentSnapshot doc in snapshot.docs) {
        count = count + (doc.data() as Map<String, dynamic>)['count'] as int;
      }
    });
    return count;
  }
  ...
  Future<List<Log>> getLogsOfVehicle({String? licensePlate}) async {
    ...
    for (var element in querySnapshot.docs) {
      allLogs.add(Log.fromMap(element.data() as dynamic));
    }
  ...
  Future<List<Vehicle>> getAllVehicles() async {
    ...
    for (var element in querySnapshot.docs) {
      allVehicles.add(Vehicle.fromMap(element.data() as dynamic));
    }
    return allVehicles;
  }
